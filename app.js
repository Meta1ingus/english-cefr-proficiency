...
// In a real application, you would define API_BASE_URL based on your environment.
...
document.getElementById("startBtn").addEventListener("click", async () => {
    ...
    if (!name) {
        ...
        resultDiv.textContent = "Please enter your name to begin.";
        return;
    }
    ...
    try {
        ...
        // Start first question (already called by loadQuestions)
        // nextQuestion(); // No need to call here, loadQuestions already calls it.
    } catch (err) {
        console.error("Registration error:", err);
        const resultDiv = document.getElementById("result");
        resultDiv.classList.remove("d-none");
        resultDiv.classList.add("alert-danger");
        resultDiv.textContent = "Something went wrong during registration — please try again.";
    }
});
...
async function loadQuestions() {
    try {
        ...
        reviewLog = []; // Clear review log on restart
        ...
        quizCard.classList.remove("d-none"); // Ensure it's visible to show the error
        quizCard.innerHTML = `<div class="alert alert-danger" role="alert">
            Failed to load quiz data. Please ensure the backend server is running at ${API_BASE_URL}.
        </div>`;
    }
}
...
function renderQuestion(q) {
    ...
    if (q.answerType === "spoken-response") {
        ...
        `;
    } else if (q.answerType === "multiple-choice" || q.choices?.length) {
        if (q.choices?.length) {
...
function checkAnswer() {
    const result = document.getElementById("result");
    result.classList.remove("d-none", "alert-info", "alert-success", "alert-danger", "alert-warning", "alert-secondary"); // Clear previous states
    ...
    if (currentQuestion.answerType === "spoken-response") {
        ...
        if (isCorrect) {
            ...
            result.classList.add("alert-danger");
            result.textContent = `❌ Incorrect. The correct answer was: ${currentQuestion.correctAnswer}`;
        }
    }
...
async function showSummary(userId) {
    try {
        ...
        reviewLog.forEach((entry, i) => {
            ...
            li.className = `list-group-item p-3 mb-2 rounded-lg ${entry.correct === true
            ...
            li.innerHTML = `
                <p class="font-semibold mb-1">Q${i + 1}: ${entry.question}</p>
                <p class="text-sm mb-1"><strong>Your Answer:</strong> ${entry.userAnswer}</p>
                ${entry.correctAnswer && entry.correctAnswer !== "Evaluated after submission" ? `<p class="text-sm"><strong>Correct Answer:</strong> ${entry.correctAnswer}</p>` : ""}
            `;
...
async function generatePdfSummary() {
    ...
    const buttonsDiv = summaryCard.querySelector('.flex.justify-center.gap-4');
    if (buttonsDiv) {
    ...
    html2canvas(summaryCard, { scale: 2, useCORS: true }).then(canvas => {
        ...
        resultDiv.classList.add("alert-danger");
        resultDiv.textContent = "❌ Failed to generate PDF. Please try again.";
    });
}
...
document.getElementById("restartBtn").addEventListener("click", () => {
    ...
    document.getElementById("namePrompt").classList.remove("d-none"); // Show name prompt again
    document.getElementById("quizCard").classList.add("d-none"); // Hide quiz card
    ...
    document.getElementById("nameInput").focus(); // Focus on name input
    // loadQuestions(); // Do not call here, it will be called after registration
});
